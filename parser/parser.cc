...
void VCDParser::get_vcd_scope() {
    ...
    vcd_module.clear();
    vcd_signal_info_table_.clear();
    vcd_signal_info_tree_.clear();
    std::ifstream file;
    ...
    while (getline(file, read_string)) {
        if (read_string.c_str()[0] == '$' && read_string.c_str()[1] == 's') {
            ...
            if (vcd_signal_info_table_.empty() != 1)
                vcd_signal_info_tree_.emplace(std::pair<std::string,
                                                        std::unordered_map<std::string, struct VCDSignalStruct>>
                                                  (vcd_module.back(), vcd_signal_info_table_));
            ...
            vcd_signal_info_table_.insert(std::pair<std::string, struct VCDSignalStruct>(signal.vcd_signal_label,
                                                                                         signal));
...
//    }
//    for (auto & iter : vcd_signal_info_tree_) {
...
void VCDParser::get_vcd_signal_flip_info(uint64_t begin_time, uint64_t end_time) {
    ...
    while (fgets(buf, sizeof(buf), fp) != nullptr) {
        ...
        if (buf[0] == 'b') {
            ...
            std::string signal_value = bufs.substr(1, bufs.find_first_of(' '));
            int count = 0;
            for (int i = 1; i < signal_value.length(); i++) {
                std::string temp_alias = signal_alias + std::to_string(count);
                cnt.last_level_status = buf[count + 1];
    ...
    while (fgets(buf, sizeof(buf), fp) != nullptr) {
        ...
        if (buf[0] == 'b') {
            ...
            for (int i = 1; i < signal_value.length(); i++) {
                ...
                if (buf[count + 1] != iter->second.last_level_status) {
                ...
                if (buf[count + 1] == 'x' || time_difference == 0 || iter->second.last_level_status == 'x')
                    iter->second.total_invert_counter--;

                if (time_difference == 0) {
                    if (burr_hash_table.find(temp_alias) == burr_hash_table.end())
                        burr_hash_table.insert(std::pair<std::string, uint16_t>(temp_alias, 0));
                    burr_hash_table.find(temp_alias)->second++;
                    if (buf[count + 1] == 'x' || iter->second.last_level_status == 'x')
                        burr_hash_table.find(temp_alias)->second--;
                }
                iter->second.last_level_status = buf[count + 1];
